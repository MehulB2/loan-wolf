...
export default function Leaderboard({ entries, currentPlayerName }: Props) {
  ...
  return (
    <div className="space-y-1">
      <div className="grid grid-cols-5 gap-2 px-3 py-2 text-xs text-slate-500 font-mono uppercase tracking-wider">
        <span>#</span>
      ...
      {entries.map((entry, index) => {
        ...
        return (
          <div
            key={index}
            className={`grid grid-cols-5 gap-2 px-3 py-2 rounded-lg font-mono text-sm transition-colors ${
              isCurrentPlayer
